...
class CreateElement {
  ...
  static createRadio(id, classList, groupName, buttonValue, displayValue, checked, handler) {
   ...
   static createTable(id, classList, headers, contents, captionLabel, attachContents, colgroupinfo) {
    ...
    if (colgroupinfo) {
      var colgroup = CreateElement._createElement('colgroup', null, null);
      table.appendChild(colgroup);
      for (var i = 0; i < colgroupinfo.length; i++) {
        var col = CreateElement._createElement('col', null, null);
        colgroup.appendChild(col);
        col.style = colgroupinfo[i];
      }
    }
